  ...
  return (
    <section id="projects" className="section-pad">
      <div className="container-md">
        ...
        {/* ── Bento Grid: max 3 latest ── */}
        <motion.div layout className="grid grid-cols-1 md:grid-cols-3 gap-5 lg:gap-6">
          <AnimatePresence mode="popLayout">
            {loading && Array.from({ length: 3 }).map((_, i) => (
              <motion.div
                ...
                animate={{ opacity: 1 }}
                exit={{ opacity: 0 }}
                className={`glass-panel animate-pulse ${i === 0 ? "md:col-span-2 md:row-span-2 min-h-[520px]" : "min-h-[250px]"}`}
              >
                <div className="w-full h-full bg-white/5 rounded-2xl" />
              </motion.div>
            ))}

            {!loading && filtered.slice(0, 3).map((p, i) => {
              const isHero = i === 0;
              return (
                <motion.a
                  key={p._id}
                  href={p.live || `/projects/${p._id}`}
                  ...
                  className={`group relative overflow-hidden rounded-2xl border border-white/10 bg-white/[0.02] cursor-pointer
                    ${isHero ? "md:col-span-2 md:row-span-2 min-h-[360px] md:min-h-[520px]" : "md:col-span-1 min-h-[220px] md:min-h-[250px]"}
                  `}
                >
                  {/* Image */}
                  ...
                  <div className={`absolute bottom-0 left-0 right-0 z-10 ${isHero ? "p-8 md:p-10" : "p-6"}`}>
                    ...
                    <div className="flex items-center gap-4">
                      {p.live && (
                        <span className="flex items-center gap-1.5 font-medium text-[13px] text-white hover:text-brand-400 transition-colors">
                          {isHero ? "Voir le projet" : "Live"} <ArrowUpRight size={14} />
                        </span>
                      )}
                      {p.github && (
                        <span className="flex items-center gap-1.5 font-medium text-[13px] text-white/50 hover:text-white transition-colors">
                          <Github size={14} /> {isHero ? "Source" : ""}
                        </span>
                      )}
                    </div>
                  </div>
